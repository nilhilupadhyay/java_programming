...
public class secondLowestSalary {
    ...
    public static Employee[] getEmployeeWithSecondLowestSalary(Employee[] e) {
        ...
        for (int i = 0; i < e.length; i++) {
            for (int j = i + 1; j < e.length; j++) {  // ✅ Fix: j = i+1, not j = i
        ...
        // Collect all employees with second lowest salary
        Employee[] det = new Employee[0];
        for (int i = 0; i < e.length; i++) {  // ✅ Fix: scan all, not just from det[0]
            if (e[i].getSalary() == secondLowest) {
